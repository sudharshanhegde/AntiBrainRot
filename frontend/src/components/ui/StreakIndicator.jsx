// The daily-streak indicator: a flame glyph paired with
// the day count, register-style monospace like every other number in the
// app. The flame is a graphic SVG element in the token system (warm
// accent), deliberately not an emoji character — text content stays
// emoji-free; this is a UI graphic, kept distinct from any string.
//
// size: "sm" for the topic screen's left zone, "lg" for the profile
// screen. When signed out (no streak), count can be null and the
// indicator renders a quiet "0".

const FLAME_PATH =
  "M13.5 0.67s0.74 2.65 0.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l0.03-0.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5 0.67zM11.71 19c-1.78 0-3.22-1.4-3.22-3.14 0-1.62 1.05-2.76 2.81-3.12 1.77-0.36 3.6-1.21 4.62-2.58 0.39 1.29 0.59 2.65 0.59 4.04 0 2.65-2.15 4.8-4.8 4.8z";

export function StreakIndicator({ count, size = "sm", label = "day streak" }) {
  const iconSize = size === "lg" ? 26 : 16;
  const textSize =
    size === "lg" ? "text-3xl font-semibold" : "text-[15px] font-medium";
  const num = typeof count === "number" ? count : 0;

  return (
    <span
      className="inline-flex items-baseline gap-2"
      title={label}
      aria-label={`${label}: ${num}`}
    >
      <svg
        width={iconSize}
        height={iconSize}
        viewBox="0 0 24 24"
        fill="var(--accent-flame)"
        className="self-center"
        aria-hidden="true"
      >
        <path d={FLAME_PATH} />
      </svg>
      <span
        className={`font-mono ${textSize} tracking-tight ${
          num > 0 ? "text-accent-complete" : "text-muted"
        }`}
      >
        {num}
      </span>
    </span>
  );
}
