...
// Thomas GIA-style cognitive questions: number series, letter series,
// odd-one-out, and word analogies. Each concept card teaches the approach,
// each quiz applies it to a fresh example.
const cognitiveAbility = [
  {
    ...
    type: "concept",
    template: "text_only",
    title: "Number series: read the gaps between terms",
    body: "A number series is a sequence governed by a hidden rule, and the fastest way to find the rule is to write down the differences between consecutive terms. In 2, 5, 10, 17, 26 the gaps are 3, 5, 7, 9, odd numbers climbing by two, so the next gap is 11 and the next term is 37. When the first differences do not settle into a pattern, difference them again, and the second differences often expose squares or cubes. Some series hide the rule in the terms themselves rather than the gaps, such as 1, 4, 9, 16, 25, which are the squares. Others alternate operations, adding and multiplying in turns, or mix two interleaved sequences. Always test any rule you propose against at least the last two gaps before committing. Speed comes from recognizing the common families, arithmetic progressions, squares, cubes, primes, and alternating operations, so you can name the pattern instead of guessing at it.",
    code_snippet: null,
    diagram_ref: null,
  },
  {
    ...
    type: "quiz",
    tests_card_id: 0,
    question: "What comes next in the series 3, 6, 11, 18, 27, ?",
    options: [
      { id: "a", text: "38" },
      { id: "b", text: "36" },
      { id: "c", text: "40" },
  ...
  },
  {
    ...
    type: "concept",
    template: "text_only",
    title: "Letter series are the alphabet as numbers",
    body: "A letter series works exactly like a number series, with the alphabet acting as the number line. Convert each letter to its position, A is 1, B is 2, and so on, then study the gaps between positions. In A, C, F, J the positions are 1, 3, 6, 10, and the gaps are 2, 3, 4, climbing by one, so the next gap is 5 and the next position is 15, the letter O. When a series skips a fixed number of letters the gap is constant, as in B, D, F, H. Watch for patterns that wrap past Z and restart at A, and for series that alternate between two rules, such as advancing by two and then by three. Writing the positions down removes the guesswork and turns a pattern in letters into the same arithmetic you already use for numbers. Only the symbols change, the reasoning is identical.",
    code_snippet: null,
    diagram_ref: null,
  },
  {
  ...
  },
  {
    ...
    type: "concept",
    template: "text_only",
    title: "Odd one out: state the rule before you pick",
    body: "An odd-one-out question gives a set that obeys one rule except for a single term that breaks it. The trick is to name the rule precisely before choosing. In 3, 5, 7, 9, 11 all terms are odd, but nine is the only composite, the rest are prime, so nine is the odd one out. In 2, 4, 8, 16, 32 the rule is doubling and every term obeys it, so there is no intruder unless one was designed in. Check the obvious rule first, parity, divisibility, or squares, then look for a subtler one such as primes, cubes, or digit sums. When two rules both fit, the assessment intends the more specific one. Stating the rule out loud forces you to find the term that cannot be described by it, which is the entire question. The answer is the term that fails the rule the whole set follows.",
    code_snippet: null,
    diagram_ref: null,
  },
  {
    ...
    type: "quiz",
    tests_card_id: 4,
    question: "Which number is the odd one out: 2, 3, 5, 7, 10?",
    options: [
      { id: "a", text: "2" },
      { id: "b", text: "3" },
      { id: "c", text: "7" },
  ...
  },
  {
    ...
    type: "concept",
    template: "text_only",
    title: "Verbal odd one out: group by category",
    body: "A verbal odd-one-out question lists words that share a category, with one word that does not fit. The cleanest approach is to classify every word and find the class with a single member. In apple, banana, carrot, cherry three are fruits and one is a vegetable, so carrot is the odd one out. The categories can be broad, such as living things, or narrow, such as mammals versus reptiles, and the intended answer is usually the one where the misfit is unambiguous. Watch for words that are near but not exact members, like a fish among mammals, and for sets where two groupings are possible, in which case the assessment favors the most natural category. If a word belongs to a class with more than one member, it cannot be the odd one out. Name the category and the intruder becomes obvious.",
    code_snippet: null,
    diagram_ref: null,
  },
  {
    ...
    type: "quiz",
    tests_card_id: 6,
    question: "Which word is the odd one out: dog, cat, lion, tiger, fish?",
    options: [
      { id: "a", text: "dog" },
      { id: "b", text: "cat" },
      { id: "c", text: "lion" },
  ...
  },
  {
    ...
    type: "concept",
    template: "text_only",
    title: "Analogies transfer one relationship to another pair",
    body: "An analogy gives a pair of words with a clear relationship and asks you to complete a second pair with the same relationship. In doctor is to hospital, the doctor works inside the hospital, so teacher is to school, the teacher works inside the school. Name the relationship precisely before looking at the options, because several answers can look plausible under a loose reading. Common relationships include place, a pilot flies a plane, purpose, a knife cuts, and part to whole, a wheel is part of a car. The relationship can run in reverse, hospital is to doctor as school is to teacher, and the order of the second pair must match the first. When the options are close, test each one with the same sentence you used for the original pair, and the matching answer is the one the sentence still supports.",
    code_snippet: null,
    diagram_ref: null,
  },
  {
    ...
    type: "quiz",
    tests_card_id: 8,
    question: "Doctor is to hospital as teacher is to what?",
    options: [
      { id: "a", text: "school" },
      { id: "b", text: "book" },
      { id: "c", text: "student" },
